...
Bridge.define('TestResultController', {
    planId: 0,
    constructor: function (table, planId) {
        this.planId = planId;
    },
    convert: function (data) {
        return Bridge.get(TestResultModel).cloneExisting(Bridge.cast(data, TestResult));
    },
    errors: function (error) {
        Bridge.get(window).alert(error);
    }
});

Bridge.define('TestResultModel', {
    inherits: [TestResult],
    statics: {
        cloneExisting: function (template) {
            ...
            model.items = Bridge.coalesce(template.items, Bridge.Array.init(0, null));
    ...
    intialise: function () {
        if (this.items.length === 0)
            ...
            this.summary = this.items.length.toString() + " Errors";
    },
...
Bridge.define('GlobalClass', {
    statics: {
        config: {
            init: function () {
                Bridge.ready(this.onReady);
            }
        },
        onReady: function () {
            Bridge.global.alert("Ready");
        }
    }
});
